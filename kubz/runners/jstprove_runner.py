...
class JSTProveRunner:
    ...
    def generate_witness_sliced(
        ...
        for segment_idx in range(num_segments):
            ...
            segment_model_path = os.path.join(
                slices_dir, segment_name, segment_model_name
            )
            segment_input_path = os.path.join(
                slices_dir, segment_name, f"{segment_name}_input.json"
            )
            segment_output_path = os.path.join(
                slices_dir, segment_name, f"{segment_name}_output.json"
            )
            segment_witness_path = os.path.join(
                slices_dir, segment_name, f"{segment_name}_witness.compiled"
            )
            ...
            try:
                process = subprocess.run(
                    cmd, capture_output=True, cwd=self.jstprove_project_path, text=True
                )
    ...
    def prove_slices(self) -> bool:
        ...
        for segment_idx in range(num_segments):
            ...
            segment_proof_path = os.path.join(
                slices_dir, segment_name, f"{segment_name}_proof.bin"
            )
            segment_witness_path = os.path.join(
                slices_dir, segment_name, f"{segment_name}_witness.compiled"
            )
    ...
    def verify_slices(self, input_path: str = None) -> dict:
        ...
        if input_path is not None:
            segment_0_input_path = os.path.join(
                slices_dir, "segment_0", "segment_0_input.json"
            )
            try:
                with (
                    open(input_path, "r") as src,
                    open(segment_0_input_path, "w") as dst,
                ):
    ...
    def circuitize_whole(self):
        ...
        memory = float(
            next(
                ...
                if "Peak Memory used Overall" in line
            ).split()[0]
        )
        time = float(
            next(
    ...
    def circuitize_slices(self):
        ...
        max_memory = max(
            segment_results[segment_idx]["memory"] for segment_idx in segment_results
        )
        total_time = sum(
            segment_results[segment_idx]["time"] for segment_idx in segment_results
        )
        segment_times = {
            segment_idx: segment_results[segment_idx]["time"]
            for segment_idx in segment_results
        }
